  ...
  private void writeResponse(MessageEvent e) throws IOException {
    ...
    try {
      ...
      if (null != httpsqs_input_name && null != httpsqs_input_opt && httpsqs_input_name.length() <= 256) {
        ...
        if (httpsqs_input_opt.equals("put")) {
          if (null != _app._conf.auth && !_app._conf.auth.equals(httpsqs_input_auth)) {
            ...
            if (_request.getMethod().getName().equalsIgnoreCase("POST")) {
              final long queue_put_value = _app.httpsqs_now_putpos(httpsqs_input_name);
              if (queue_put_value > 0) {
                final String queue_name = String.format("%s:%d", httpsqs_input_name, queue_put_value);

                final String httpsqs_input_postbuffer = URLDecoder.decode(_request.getContent().toString(_charsetObj), charset);
                final DatabaseEntry key = new DatabaseEntry(queue_name.getBytes(Sqs4jApp.DB_CHARSET));
                final DatabaseEntry value = new DatabaseEntry(httpsqs_input_postbuffer.getBytes(Sqs4jApp.DB_CHARSET));
                final OperationStatus status = _app._db.put(null, key, value);
              ...
              if (queue_put_value > 0) {
                ...
                final DatabaseEntry key = new DatabaseEntry(queue_name.getBytes(Sqs4jApp.DB_CHARSET));
                final DatabaseEntry value = new DatabaseEntry(httpsqs_input_data.getBytes(Sqs4jApp.DB_CHARSET));
                final OperationStatus status = _app._db.put(null, key, value);
                if (status == OperationStatus.SUCCESS) {
                  response.setHeader("Pos", queue_put_value);
                  _buf.append("HTTPSQS_PUT_OK");
          ...
          if (null != _app._conf.auth && !_app._conf.auth.equals(httpsqs_input_auth)) {
            ...
            final long queue_get_value = _app.httpsqs_now_getpos(httpsqs_input_name);
            if (0 == queue_get_value) {
              _buf.append("HTTPSQS_GET_END");
            } else {
              final String queue_name = String.format("%s:%d", httpsqs_input_name, queue_get_value);

              final DatabaseEntry key = new DatabaseEntry(queue_name.getBytes(Sqs4jApp.DB_CHARSET));
              final DatabaseEntry value = new DatabaseEntry();
              final OperationStatus status = _app._db.get(null, key, value, LockMode.DEFAULT);
        ...
        } else if (httpsqs_input_opt.equals(Sqs4jApp.KEY_MAXQUEUE) && httpsqs_input_num >= 10 && httpsqs_input_num <= Sqs4jApp.DEFAULT_MAXQUEUE) {
          if (checkUser(response)) {
            if (0 != _app.httpsqs_maxqueue(httpsqs_input_name, httpsqs_input_num)) {
